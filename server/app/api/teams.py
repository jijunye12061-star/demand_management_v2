...
def list_teams(db: DB, admin: AdminUser):
    ...
    for t in teams:
        org_count = db.execute(
            select(func.count(TeamOrgMapping.id)).where(TeamOrgMapping.team_id == t.id)).scalar() or 0
        member_count = db.execute(select(func.count(User.id)).where(User.team_id == t.id)).scalar() or 0
        result.append(
            TeamResponse(id=t.id, name=t.name, created_at=t.created_at, org_count=org_count, member_count=member_count))
    return result
